      ...
      <div class="card-body table-responsive">
        ...
        <table id="dtHorizontalExample" class="table table-condensed table-hover" cellspacing="0" width="100%" >
          ...
          <tbody>
              ...
              <tr>
                <th scope="row"><a href="#">{{$paintjob->user_id}}</a></th>
                <td>{{$paintjob->first_name}} {{$paintjob->last_name}}</td>
                <td>{{ substr_replace(substr_replace($paintjob->contact, '-', 4, 0), '-', 8, 0) }}</td>
                <td >{{$paintjob->car_year}}</td>
                <td style="text-transform: capitalize">{{$paintjob->car_make}}</td>
                <td style="text-transform: capitalize">{{$paintjob->car_model}}</td>
                <td style="text-transform: uppercase">{{$paintjob->car_variant}}</td>
                <td style="text-transform: uppercase">{{$paintjob->unit_plate_no}}</td>
                <td>{{$paintjob->special_request ?? 'no special request'}}</td>
                <td>{{ strtoupper(\Carbon\Carbon::parse($paintjob->date)->format('F-d-Y')) }}</td>
                <td>{{$paintjob->time}}</td>
                <td class="text-center">
                  @if ($paintjob->images && count($paintjob->images) > 0)
                  <button type="button" class="btn btn-primary show-images-btn" data-bs-toggle="modal" data-bs-target="#imagesModal" data-images="{{ json_encode($paintjob->images) }}">
                      <i class="fa-solid fa-image"></i>
                  </button>
              @endif
                </td>
                <td class="text-center">
                  <button type="button" class="btn btn-danger" onclick="showConfirmationModal('Reject', {{$paintjob->id}})"><i class="fa-solid fa-user-xmark"></i></button>
                  <button type="button" class="btn btn-success" onclick="showConfirmationModal('Accept', {{$paintjob->id}})"><i class="fa-solid fa-user-check"></i></button>
                  <form id="confirmForm_{{$paintjob->id}}" action="" method="POST" style="display: none;">
                      @csrf
                      @method('POST')
                  </form>
                </td>
...
<script>
  function showConfirmationModal(action, id) {
    ...
    const formAction = action === 'Accept'
        ? "{{ route('system.paintjob.toPayment', ['id' => ':id']) }}".replace(':id', id)
        : "{{ route('system.paintjob.phase_1', ['id' => ':id']) }}".replace(':id', id);
